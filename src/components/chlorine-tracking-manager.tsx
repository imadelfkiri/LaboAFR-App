...
export function ChlorineTrackingManager() {
  ...
  const [latestAfGoFlow, setLatestAfGoFlow] = useState<number>(0);
  ...
  const fetchLatestMixtureData = useCallback(async () => {
      try {
          ...
          if (session) {
              ...
              setLatestAfGoFlow(afFlow + goFlow);
          }
      } catch (error) {
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="grid md:grid-cols-4 gap-4 items-end">
            ...
            <div className="space-y-2 p-2 border rounded-md bg-muted/50">
              <label className="text-sm font-medium text-muted-foreground">Débit AF+GO (auto)</label>
              <p className="font-bold text-lg">{latestAfGoFlow.toFixed(2)} t/h</p>
            </div>
            <div className="space-y-2 p-2 border rounded-md bg-muted/50">
        ...
        <CardContent>
            <ResponsiveContainer width="100%" height={400}>
                ...
                <ComposedChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                    ...
                    <Line yAxisId="right" type="monotone" dataKey="totalAfGoFlow" name="Débit AF+GO" stroke="#ffc658" strokeDasharray="5 5" />
                </ComposedChart>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="text-right">Débit AF+GO (t/h)</TableHead>
                    ...
                    <TableBody>
                        {entries.map(entry => (
                            <TableRow key={entry.id}>
                                ...
                                <TableCell className="text-right">{entry.totalAfGoFlow.toFixed(2)}</TableCell>
                         ...
                         {entries.length === 0 && !loading && (
                            <TableRow><TableCell colSpan={5} className="text-center h-24">Aucune donnée.</TableCell></TableRow>
                        )}
                    </TableBody>
                </Table>
            </div>
